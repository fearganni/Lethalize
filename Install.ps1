...
function Install ($arguments) {
    Write-Host "Thanks to Daddy Ganni 🙏"
    Write-Host ""
    ...
    # Download and install biggerlobby
    Write-Host "Downloading and installing MoreSuits"
    $moreSuitsVersion = Get-Arg $arguments "-moresuits"
    $moreSuitsUrl = "https://thunderstore.io/package/download/x753/More_Suits/$moreSuitsVersion/"
    $moreSuitsStream = Request-Stream $moreSuitsUrl
    Expand-Stream $moreSuitsStream $lethalCompanyPath
    Write-Host "Installed MoreSuits"
    Write-Host ""
}
